...
class Decks {
    ...
    public Decks(int n, boolean withJokers)
    {
    	...
    	for (int currentDecks = 0; currentDecks < numberDecks; currentDecks++) {
    		for (int currentRank = 1; currentRank <= 13; currentRank++) {
    			for (int currentSuit = 1; currentSuit <= 4; currentSuit++) {
    				try {
    					originalDecks.add(new Card(currentSuit, currentRank));
    				}
    				catch (PlayingCardException e) {
    ...
    public List<Card> deal(int numberCards) throws PlayingCardException
    {
    	...
    	if (numberCards > gameDecks.size()) {
    		throw new PlayingCardException("Not enough cards left in deck!");
    	}
